...
export class SeguimientoComponent implements OnInit {
  ...
  dataTypes: any[];
  dataPrint: any = [];
  dataBaseFilter: String = 'La Joya Mining';
  dbSelect: any = [
    { value: 'Lajoya_2020', name: 'La Joya Mining' },
    { value: 'Lajoya_Concesiones', name: 'La Joya Concesiones' }
  ];
  obj_EditGeneral: any = {
    authorized: false,
    date2: '',
  ...
  constructor(
    ...
    this.formPrint = new FormGroup({
      ...
      receiver: new FormControl('PLANTA - La Joya Mining SAC'),
      starting_point: new FormControl('Oficina - AQP'),
      arrival_point: new FormControl('PLANTA - San Jose'),
  ...
  ngOnInit() {
    ...
    if (this.identity === null) {
      ...
      this.accessLevel = this.accessControl.checkAccess(this.identity.strUsuario, 'ACC-004');
      if (this.accessLevel) {
        this.bind();
      }
    }
  ...
  searchOdooSeguimientos() {
    ...
    this.spinner.show();
    this.formSearch.patchValue({ database: this.dataBaseFilter })
    this.seguimientoService.searchOdooSeguimientos(this.formSearch.value, this.pageSize, this.pageIndex).subscribe(
      response => {
  ...
  cargarDatosSeguimiento(item) {
    this.spinner.show();
    this.itemEdit = item;
    this.seguimientoService.getSeguimiento(item.lic_id, item.lic_det_id, item.prod_id, this.dataBaseFilter).subscribe(
      response => {
        ...
        this.spinner.hide();
        this.global.showNotification(error);
      }
    )
  }
  ...
  upSeguimiento(idSeg, Data) {
    this.spinner.show();
    this.seguimientoService.updateSeguimiento(Data, idSeg).subscribe(
      response => {
  ...
  addSeguimiento(Data) {
    this.spinner.show();
    this.seguimientoService.insertSeguimiento(Data).subscribe(
      response => {
        this.displayEdit = false;
        this.global.showNotification(response['message'], 'success');
        this.spinner.hide();
  ...
  promiseGeneral(item) {
    if (item.id_seg_mongo === '-1') {
      ...
      return this.seguimientoService.insertSeguimiento(seguimiento).subscribe(
      ...
      return this.seguimientoService.updateSeguimiento(upSeguim, item.id_seg_mongo).subscribe(
        response => { return response; },
        error => {
          console.error(error.error.message, 'danger');
          return '-1';
  ...
  exportFullData(): void {
    ...
    this.seguimientoService.downloadOdooSeguimiento(data).subscribe(
      response => {
  ...
  handleSendMail() {
    ...
    let conc_msg = '';
    for (let i = 0; i < dataSelect.length; i++) {
      // conc_msg += '<li>' + this.global.getNumberFix(dataSelect[i].cantidad, 2) + ' ' + dataSelect[i].uom + ' de <b>' + dataSelect[i].descripcion + '.</b></li>';
      conc_msg += '- ' + this.global.getNumberFix(dataSelect[i].saldo_pedir, 2) + ' ' + dataSelect[i].uom + ' de ' + dataSelect[i].descripcion + '.\n';
    }
    // let _message =
    // `<font color="#385d8b">Estimados Señores<br><br>
  ...
  imprimirGuia() {
    let _fPrint = this.formPrint.value;
    let _split = (_fPrint.date).split('-');
    let _table = this.dataPrint;

    let doc = new jsPDF('p', 'mm', 'a4');
    doc.setFontSize(7.5);
    doc.text(43, 46, (_split.length === 3) ? _split[2] : ''); // Fecha: DIA
    doc.text(58, 46, (_split.length === 3) ? _split[1] : ''); // Fecha: MES
    doc.text(72, 46, (_split.length === 3) ? _split[0] : ''); // Fecha: AÑO
    doc.text(40, 52, _fPrint.receiver.toUpperCase()); // Destinatario
    doc.text(40, 59, _fPrint.starting_point.toUpperCase()); // Punto partida
    doc.text(40, 66, _fPrint.arrival_point.toUpperCase()); // Punto llegada
    doc.text(130, 72, _fPrint.doc_identify); // Ruc o Doc
    doc.text(180, 72, _fPrint.number_invoice.toUpperCase()); // Nro Fact.

    doc.text(30, 275.5, _fPrint.vehicle_mark.toUpperCase()); // Marca Vehiculo
    doc.text(80, 275.5, _fPrint.plate_number.toUpperCase()); // Placa
    doc.text(135, 275.5, _fPrint.constancy_ins.toUpperCase()); // Constancia
    doc.text(180, 275.5, _fPrint.license_number.toUpperCase()); // Nro Licencia
    doc.text(30, 282, _fPrint.carrier_name.toUpperCase()); // Transportista
    doc.text(170, 282, _fPrint.carrier_ruc.toUpperCase()); // Ruc Transp.

    let yValue = 99;
    _table.forEach((item) => {
      doc.text(12, yValue, (item.description) ? item.description : ''); // DECSRIPCION
      doc.text(125, yValue, (item.unit) ? item.unit : ''); // UNIDAD
      doc.text(145, yValue, (item.qty) ? item.qty.toString() : ''); // CANTIDAD
      doc.text(162, yValue, (item.number_lic) ? item.number_lic : ''); // NRO LICITACION
      doc.text(180, yValue, (item.purchase_order) ? item.purchase_order : ''); // NRO PO
      yValue += 6;
    });
    doc.save('Guia_' + new  Date().getTime() + '.pdf');
  }
  loadDataPrint() {
    let dataSelect = this.gridData.filter(x => x.check_seg === true);
    this.dataPrint = [];
    for (let i = 0; i < 29 ; i++) {
      if (dataSelect[i]) {
        const item = {
          description: (dataSelect[i].prod_code) ? '[' + dataSelect[i].prod_code + '] ' + dataSelect[i].prod_name : dataSelect[i].prod_name,
          unit: dataSelect[i].uom,
          qty: dataSelect[i].cantidad,
          number_lic: dataSelect[i].lic_number,
          purchase_order: dataSelect[i].po_name
        };
